...
| `tools/mine_conceitos.py` | **Minera conceitos contextuais** (§4.2) → backlog `wiki/lacunas-conceitos.md` a promover para `conceitos/`. Dois sinais: **`--neologisms`** (recomendado) varre TODAS as fontes por neologismos *em itálico* recorrentes sem página, ranqueando por convergência entre fontes; **`--book dac`** detecta o conceito-núcleo embutido nas `-logia` (reusa `core_concept`). Sempre só sugere — relevância/alias é julgamento do LLM/curador. | `python tools/mine_conceitos.py --neologisms` |
